...
export function PasscodeSection({
  ...
  const submit = (e: React.FormEvent): void => {
    ...
    setMessage(null)
    if (next !== confirm) {
      setMessage({ ok: false, text: 'New passcodes do not match.' })
      return
    }
    void window.godfirst.security
      .setPasscode(hasPasscode ? current : null, next)
      .then((result) => {
        if (result.ok) {
          setMessage({ ok: true, text: hasPasscode ? 'Passcode changed.' : 'Passcode set.' })
          setCurrent('')
          setNext('')
          setConfirm('')
          onChanged()
        } else {
          setMessage({ ok: false, text: result.error ?? 'Could not set passcode.' })
        }
      })
  }
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3">
      <div>
        <h2 className="text-sm font-medium">Recovery passcode</h2>
        <p className="mt-1 text-xs text-neutral-500">
          The emergency escape from a locked screen — the only way out besides finishing the
          reading. {hasPasscode ? 'A passcode is currently set.' : 'No passcode set yet; GodFirst will not lock without one.'}
        </p>
      </div>
      {hasPasscode && (
        <input
      ...
      <button
        type="submit"
        className="mt-1 self-start rounded bg-neutral-100 px-4 py-1.5 text-sm font-medium text-neutral-900 hover:bg-white disabled:opacity-40"
